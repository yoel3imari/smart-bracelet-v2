...
import {
  ...
  validateUuidConfig,
  type BleUuidConfig
} from '@/config/ble-uuid-config';

// Test data for UUID discovery
const testServices = ['6E400001-B5A3-F393-E0A9-E50E24DCCA9E'];
...
class BleConnectionTest {
  ...
  async testPromiseRejectionHandling() {
    ...
    try {
      ...
      (fixedError as any).reason = mockError.reason || 'Unknown reason';
      
      console.log('Fixed Error Object:', {
        message: fixedError.message,
        code: (fixedError as any).code,
        reason: (fixedError as any).reason
      });
      
      // Verify the error object has all required properties
      if (fixedError.message && (fixedError as any).code && (fixedError as any).reason) {
  ...
  async testCharacteristicMonitoring() {
    ...
    try {
      ...
      const testCallback = (error: any | null, characteristic: any | null) => {
        if (error) {
  ...
  async testConnectionTimeoutHandling() {
    ...
    try {
      ...
      const deviceConnection = new Promise<any>((resolve) => {
...
export default BleConnectionTest;